import { notion, drive, model, embeddings, textSplitter } from './clients'
...
// LLMを使用した検索結果の処理
async function processWithLLM(results: SearchResult[], query: string): Promise<string> {
  // 検索結果をテキストチャンクに分割
  ...
  const vectorStore = await MemoryVectorStore.fromDocuments(flatDocs, embeddings)
  ...
  const response = await model.call(
    `以下の情報源に基づいて、質問に答えてください。回答には関連する情報源のURLも含めてください。
